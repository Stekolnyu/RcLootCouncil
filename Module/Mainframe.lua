...
local defaults = {
	...
	profile = {
		...
		otherAwardReasons = { -- Used in "Award for ..." @ the rightclick menu
			{ text = "Дезинчант",	log = true},	--1
			{ text = "Банк",			log = true},	--2
			{ text = "Бесплатно",			log = false},	--3
		},
		autoAward = false,
		...
		dbToSend = {
			...
			buttons = {
				{	--1
					text = "Мейнспек/Нужно",
					color = {0, 1, 0,1}, -- Green
					response = "Мейнспек/Нужно",
					whisperKey = "надо, мейнспек, мс, 1",
				},
				{	--2
					text = "Оффспек/Неоткажусь",
					color = {1, 0.5, 0,1}, -- Orange
					response = "Оффспек/Неоткажусь",
					whisperKey = "greed, оффспек, ос, 2",
				},
				{	--3
					text = "Незначительное улучшение",
...
function RCLootCouncil:MainFrame_OnLoad()
	...
	StaticPopupDialogs["RCLOOTCOUNCIL_CONFIRM_ABORT"] = {
		text = "Вы уверены что хотите сделать откат?",
		button1 = "Да",
		button2 = "Нет",
		OnAccept = function()
	...
	StaticPopupDialogs["RCLOOTCOUNCIL_CONFIRM_USAGE"] = {
		text = "Вы хотите использовать RCLootCouncil для этого рейда?",
		button1 = "Да",
		button2 = "Нет",
		OnAccept = function()
		...
		OnCancel = function ()
			...
			isRunning = false;
			self:Print("Не активен в этом рейде. Вы можете включить его в настройках.")
		end,
		timeout = 0,
		enterClicksFirstButton = true,
		whileDead = true,
...
function RCLootCouncil:ChatCommand(msg)
    ...
    if not input or input:trim() == "" or input == 'help' then
		...
		self:Print("- Показать - Показать the main loot frame")
		self:Print("- Спрятать - Спрятать the main loot frame")
		...
		self:Print("- Тест (#)  - Эмулировать лут сессию (введите число для рейдового теста)")
		self:Print("- Версия - Открыть проверку версии (alt. 'v' или 'ver')")
		self:Print("- Истории - Открыть окно истории добычи")
		self:Print("- Шёпот - Показывать помощь при отправки комманд через шёпот (alt. 'his')")
		self:Print("- Сброс - Сбросить фрейм-позиции аддона")
...
function RCLootCouncil.getPlayerRole(arg)
	...
	if role == "TANK" then return "Танк";
	elseif role == "HEALER" then return "Хиллер";
	elseif role == "DAMAGER" then return "ДД";
	...
	if id then
		...
		if entryTable[currentSession][id][11] then -- if unvote
			...
			if not mlDB.selfVote and GetUnitName("player", false) == entryTable[currentSession][id][1] then -- test that they may vote for themself
				self:Print("Лутер проголосовал за себя")
				return;
			end
			for i = 1, #entryTable[currentSession] do -- test that they are not multivoting
				 if entryTable[currentSession][i][11] and not mlDB.multiVote then
					 self:Print("Лутер выключил мультиголосование")
					 return;
				 end
			end
			entryTable[currentSession][id][8] = entryTable[currentSession][id][8] + 1;
